...
console.log(user);
  ...
  return (
    <header className='w-full bg-black fixed top-0 left-0 right-0 transitio-all ease-in duration-300'>
      <nav className={`py-4 lg:px-24 px-4 ${isSticky?"sticky top-0 left-0 right-0 bg-black":""} `}>
        <div className='flex justify-between items-center text-base gap-8'>
          {/* Logo */}
          <Link to="/" className="text-2xl font-bold text-white flex items-center">
             Campus<span className='logoH'>Trade</span>
          </Link>

          {/* Nav items for large devices */}
          <ul className="md:flex space-x-12 hidden">
            {navItems.map(({ link, path }) => (
              <Link key={path} to={path} className="block text-base text-white uppercase cursor-pointer hover:text-purple-400">
                {link}
              </Link>
            ))}
          </ul>
          ...
          <div className="space-x-12 hidden lg:flex items-center text-white">
            <button>
              <FaBars className="w-5 hover:text-white mx-1 " />
            </button>
            {
              

              user ? user.email.replace('@gmail.com', "").charAt(0).toUpperCase() + user.email.replace('@gmail.com', "").slice(1) : ""
          ...
          <div className="md:hidden">
            <button onClick={toggleMenu} className='text-white focus:outline-none'>
              {isMenuOpen ? <FaXmark className="h-5 w-5 text-white" /> : <FaBars className="h-5 w-5 text-white" />}
            </button>
          </div>
        </div>
